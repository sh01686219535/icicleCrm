    ...
    public function dashboard()
    {
        ...
        if ($cmo || $gm) {
            if ($gm) {
                ...
                return view('backend.AuthUser.cmoGM', compact('teamLeaderCount', 'employee', 'gm', 'data', 'lead', 'targetMoney', 'employeeTarget', 'unpaidAmount', 'currentMonthName'));
            } elseif ($cmo) {
                $cmoId = $cmo->id;
                ...
                $lead = Lead::where('sales_officer', $cmo)->count('id');
                $activeLead = Task::whereIn('status', ['Interested', 'Appointment Schedule', 'Sure Secure'])
                    ->where('employee_name', $cmo)
                    ->count();
                $junkLead = Task::whereIn('status', ['Not Interested'])
                    ->where('employee_name', $cmo)
                    ->count();
                ...
                return view('backend.AuthUser.mainCmo', compact('teamLeaderCount', 'employee', 'cmo', 'data', 'targetMoney', 'employeeTarget', 'unpaidAmount', 'currentMonthName'));
            }
        }
        //salesMan
        if ($salesMan) {
