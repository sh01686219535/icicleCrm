                                ...
                                <div class="col-xxl">
                                    <div class="card mb-4">
                                        <div class="card-body">
                                            ...
                                            <form id="myForm2" class="form-demo" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                ...
                                                <div class="form-section dummy">
                                                    <div class="row mb-3">
                                                        ...
                                                        <div class="col-md-3">
                                                            <label for="serial_number" class="my-2">Investor</label><br>
                                                            <select name="serial_number" id="serial_number" class="form-control my-2">
                                                                <option value="">Select Serial ID</option>
                                                                @foreach ($investor as $value)
                                                                <option value="{{ $value->id }}">{{ $value->serial_number }}</option>
                                                                @endforeach
                                                                <input type="text" id="serialnumber" name="serialnumber" id="textInput1" value="">
                                                        ...
                                                        <div class="col-md-3">
                                                            <label class="form-label my-2 dummy" for="name">Applicant's Full Name<span class="text-danger" style="font-size:20px;">*</span></label>
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <input type="number" class="form-control" name="agreed_price" id="agreed_price" />
                                                        </div>
                                                        <div class="col-md-3">
...
<script>
    $(document).ready(function() {
        $('#serial_number').change(function() {
            ...
            $.ajax({
                url: '/getMultiInvestor', // Make sure this URL matches your actual URL in the web routes.
                type: 'GET',
                dataType: 'json',
                data: {
                ...
                success: function(data) {
                    ...
                    $('#spouse_date_birth').val(data.spouse_date_birth);
                    ...
                    $('#nid_passport').val(data.nid_passport);
                    $('#passport').val(data.passport);
                    $('#Nationality').val(data.nationality);
                    $('#project_name').val(data.project_name);
                    $('#project_address').val(data.project_address);
                    $('#ownership_size').val(data.ownership_size);
                    $('#category_ownership').val(data.category_ownership);
                    $('#no_ownership').val(data.no_ownership);
                    $('#price_ownership').val(data.price_ownership);
                    $('#price_ownership_word').val(data.price_ownership_word);
                    $('#totalPrice').val(data.totalPrice);
                    $('#agreed_price').val(data.agreed_price);
                    $('#agreed_price_word').val(data.agreed_price_word);
                    $('#special_discount').val(data.special_discount);
                    $('#special_discount_word').val(data.special_discount_word);
                    $('#Installment').val(data.installment);
                    $('#quarterly').val(data.quarterly);
                    $('#half_yearly').val(data.half_yearly);
                    $('#yearly').val(data.yearly);
                    $('#at_a_time').val(data.at_a_time);
                    $('#main_amount_first').val(data.main_amount_first);
                    $('#down_payment').val(data.down_payment);
                    $('#down_payment_date').val(data.down_payment_date);
                    $('#down_payment_inWord').val(data.down_payment_inWord);
                    $('#payment_type2').val(data.payment_type2);
                    $('#chq').val(data.chq);
                    $('#bank_name').val(data.bank_name);
                    $('#branch_name').val(data.branch_name);
                    $('#Online_Payment').val(data.Online_Payment);
                    $('#payment_type_date2').val(data.payment_type_date2);
                    $('#no_of_installment').val(data.no_of_installment);
                    $('#inst_per_month').val(data.inst_per_month);
                    $('#main_amount').val(data.main_amount);
                    $('#inst_start').val(data.start_from);
                    $('#inst_to').val(data.start_to);
                    $('#others_instruction').val(data.others_instruction);
                    $('#allow_amount').val(data.allow_amount);
                    $('#nominee_cell_no').val(data.nominee_cell_no);
                    $('#relation_to_nominee').val(data.relation_to_nominee);
                    $('#reference_name_a').val(data.reference_name_a);
                    $('#reference_cell_a').val(data.reference_cell_a);
                    $('#reference_name_b').val(data.reference_name_b);
                    $('#reference_cell_b').val(data.reference_cell_b);
                    $('#id_employee').val(data.id_employee);
                    $('#team_leader').val(data.team_leader);
...
<script>
    ...
    $('#team_leader').change(function() {
        ...
        $('#previewBtn').click(function() {
            const formData2 = $('#myForm2').find(':input:not([type=hidden])').serializeArray();

            localStorage.setItem('formData2', JSON.stringify(formData2));

            window.location.href = '/multiplepreview';
        });
        //investor Image
        $('#image').change('click', function(e) {
            ...
            reader.onload = function(e) {
                ...
                const imageData2 = e.target.result;
                localStorage.setItem('imageData2', imageData2);
            }
        ...
        $(document).ready(function() {
            $('#nominee_image').change('click', function(e) {
                ...
                reader.onload = function(e) {
                    ...
                    const nomimageData2 = e.target.result;
            localStorage.setItem('nomimageData2', nomimageData2);
                }
                reader.readAsDataURL(e.target.files['0']);
            });
        });
        $('#submitbtn').click(function() {
            ...
            localStorage.removeItem('nomimageData2')
        });
    });
    setTimeout(() => {
